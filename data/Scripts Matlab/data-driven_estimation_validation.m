...
folder = 'data-driven_estimation';
name = 'validate_J_';
load(strcat(folder,'/',name,jacobian,'.mat'));
...
hold on
ylim([-2 2]);
title('Estimation Error'),xlabel('Insertion length [mm]'),ylabel('Error [mm]'), legend('horizontal (X)', 'depth (Y)', 'vertical (Z)', 'Orientation','horizontal','Location', 'southeast')
...
hold on
title('Needle guide position'),xlabel('Insertion length [mm]'),ylabel('position [mm]'), legend('horizontal (X)','vertical (Z)')
